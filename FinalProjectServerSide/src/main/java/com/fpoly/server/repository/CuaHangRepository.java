...
public interface CuaHangRepository extends JpaRepository<CuaHang, Long> {
	...
	CuaHangResponse findOne(String idCuaHang, Boolean trangThai);
	...
	CuaHangResponse findByIdNguoiDung(String idNguoiDung, Boolean trangThai);
	...
	@Query(value = "SELECT ch.idCuaHang, ch.tenCuaHang, ch.idNguoiDung, nd.userName,"
			...
			+ " FROM cua_hang ch INNER JOIN nguoi_dung nd"
			+ " ON ch.idNguoiDung = nd.idNguoiDung"
			+ " WHERE ch.tenCuaHang LIKE CONCAT('%', ?1, '%') AND ch.trangThai = ?2", nativeQuery = true)
	List<CuaHangResponse> findByTenCuaHang(String tenCuaHang, Boolean trangThai);
